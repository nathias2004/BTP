	...
	while(F.size()!=0){
		...
		while(1){
			...
			if(Calculate_Size(V_dash)<PageSize){
				cout<<"The size of V_dash is: "<<V_dash.size()<<endl<<endl;
				cout<<"Number of nodes still there : "<<F.size()<<endl<<endl;
				if(F.size()>0){ //there are nodes that need to be traversed
					...
					cout<<"The size of V_dash after BFS is: "<<V_dash.size()<<endl<<endl;
					cout<<"Number of nodes still there after BFS: "<<F.size()<<endl<<endl;
				}
				else{
...
//File Size collection
	...
	cluster_nodes_into_pages(Nodes,(PageSize)/2,0);
	...
	outfile1.open("BPLUS.txt",ios_base::app);
	for (auto i : Bplus) 
        outfile1<<i.first<<" "<< i.second<<endl;

    outfile1.close();

    outfile1.open("Record_size.txt",ios_base::app);
    for(auto i:RecordSizes)
    	outfile1<<i.first<<" "<<i.second<<endl;



cout<<"The new page is: "<<New_PageNo<<endl;
